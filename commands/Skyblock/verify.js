...
module.exports = {
	...
	execute(bot, message, args) {
        ...
        message.delete();
       if (ign === undefined) {
         message.channel.send('<a:no:847468672380829707> Please use the proper Format: \`!verify IGN\`').then(msg => setTimeout(() => { msg.delete() }, 10000));
         return;
       }
        axios.get(`https://some-random-api.ml/mc?username=${ign}`)
            .then((res) => {
                const uuid = res.data.uuid
                axios.get(`https://api.hypixel.net/player?name=${ign}&key=${config.apikey}`)
                    .then((res) => {
                        if (res.data.player.socialMedia.links.DISCORD != message.author.tag) {
                            message.channel.send('Your Discord isn\'t Linked to your Hxpixel Account or you entered the wrong IGN.')

                    
                        } else if (res.data.player.socialMedia.links.DISCORD === message.author.tag) {
                            axios.get(`https://hypixel-api.senither.com/v1/profiles/${uuid}/catacombs?key=${config.apikey}`)
                            .then((res) => {
    ...
    .setDescription(`${message.author} has successfully verified and their Nickname has been updated.`)
    .setColor('GREEN')
    message.channel.send(vembed).then(msg => setTimeout(() => { msg.delete() }, 10000));
                            })
                        }
                    })
            })
