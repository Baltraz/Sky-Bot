...
module.exports = {
  ...
  if (rrxp > 1000) { rrxp = Math.floor(rxp / 1000000)+ '.' + Math.floor(rxp / 1000) + 'M'} else { rrxp = rrxp + 'k'}

  const txp = apiData.data.slayers.bosses.tarantula.experience
  let ttxp = Math.floor(txp / 1000)
  if (ttxp > 1000) { ttxp = Math.floor(txp / 1000000)+ '.' + Math.floor(txp / 1000) + 'M'} else { ttxp = ttxp + 'k'}

  const sxp = apiData.data.slayers.bosses.sven.experience
  let ssxp = Math.floor(sxp / 1000)
  if (ssxp > 1000) { ssxp = Math.floor(sxp / 1000000)+ '.' + Math.floor(sxp / 1000) + 'M'} else { ssxp = ssxp + 'k'}

  const exp = apiData.data.slayers.bosses.enderman.experience
  let eexp = Math.floor(exp / 1000)
  if (eexp > 1000) { eexp = Math.floor(exp / 1000000)+ '.' + Math.floor(exp / 1000) + 'M'} else { eexp = eexp + 'k'}
